...
- [x] 02-03-PLAN.md — Classification pure logic: CalendarEvent, CourseClassifier, FolderNameSanitizer (Wave 1)
...
**Plans**: 4/4 plans executed (Task 4 macOS device verification pending)
**UI hint**: yes
...
- [x] 03-03-PLAN.md — Vault writing + pipeline wiring: NSFileCoordinatorNoteWriter + HardcodedVaultResolver + PipelineWiring (Wave 2)
- [x] 03-04-PLAN.md — Menu-bar popover UI: MenuBarViewModel + MenuBarPopover + MenuBarExtra wiring (Wave 2) — *code complete; Task 4 macOS device verification outstanding*
...
- [x] 04-02-PLAN.md — CalendarEventProvider protocol + EventKitCalendarAdapter (macOS+iOS) + TermRangeFilter (Wave 1)
- [x] 04-03-PLAN.md — PermissionState enum + CoursePickerViewModel (search, recent, selection) (Wave 1)
...
- [x] 04-06-PLAN.md — Integration Wiring: connect ScheduleAwareVaultResolver + state observer + picker data + manage courses data (Wave 3, gap closure)
...
- [x] 05-01-PLAN.md — Onboarding flow: first-run wizard (vault picker, mic, calendar, term), PermissionsSheet, BookmarkStore
- [x] 05-02-PLAN.md — iOS capture surface: TabView shell, background recording, Now Playing, interruption handling
...
**Plans**: 6 plans
**Wave 1** (parallel, no dependencies):
- [ ] 06-01-PLAN.md — Foundation infrastructure (APIKeyStore, ConsentStore, FrontmatterSchema v2, ModelLoadGate.ollama, summary-default.md)
**Wave 2** (parallel, depends on 06-01):
- [ ] 06-02-PLAN.md — Ollama local summarization (OllamaLLMSummarizer, health check, prompt template, summary injection, Regenerate Summary)
...
- [ ] 06-06-PLAN.md — iOS Settings tab + Audit tab + UAT (iOSSettingsTab read-only, AuditTab with filters, zero-telemetry verification, local-first offline test)
