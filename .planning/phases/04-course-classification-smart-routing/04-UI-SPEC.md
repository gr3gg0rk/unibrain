...
| Component library | SwiftUI (Apple-built — inline `PopoverOverlay` view-state switching, `List`, `Section`, `TextField`, `Button`, `Label`, `Banner`) |
...
**Design language:** Native macOS menu-bar app. Follows Apple Human Interface Guidelines for `MenuBarExtra` popover surfaces and inline view-state switching via a `PopoverOverlay` enum. All Phase 4 surfaces (picker, manage courses, permission explanation, term editor) render INLINE within the 280pt popover by switching on `viewModel.overlayState` — NOT via `.sheet`. This avoids the known `MenuBarExtra` + `.sheet` bug (Apple Feedback FB11984872, see RESEARCH.md Pitfall 2) where sheets fail to anchor or appear behind other windows. No custom theming layer — uses standard semantic colors so the app automatically respects Light/Dark mode and system accent color preferences. Phase 4 extends Phase 3's popover with classification surfaces; no Phase 3 tokens change.
...
| xl | 24pt | Popover top/bottom padding, inline overlay top padding |
| 2xl | 32pt | Major section break (not used — surfaces are compact) |

**Popover width:** 280pt fixed (inherited from Phase 3 P-09).

**Inline overlay width:** Picker, Manage Courses, and all Phase 4 surfaces render inline within the 280pt popover frame by switching `viewModel.overlayState` (an inline `PopoverOverlay` enum). Stays in the menu-bar surface (MP-01 — Angelica never leaves her current app). Per RESEARCH.md Pitfall 2 (FB11984872): `.sheet` attached to `MenuBarExtra(.window)` is unreliable on macOS 15 — sheets fail to anchor or appear behind other windows. Inline view-state switching is the correct pattern.
...
|------|-------------------|-------|
| Dominant (60%) | `Color(nsColor: .windowBackgroundColor)` | Popover/inline overlay background |
...
- "Open System Settings" deep-link button in permission overlay
...
- Delete course mapping action (Manage Courses overlay)
...
### Surface 1: Manual Course Picker (inline overlay) (MP-01..MP-05)

Presented inline within the 280pt popover by setting `viewModel.overlayState = .coursePicker(mode)` when `CourseClassifier` returns `.multiple` or `.none`, or when calendar permission is denied and Angelica records. NOT presented via `.sheet` (per RESEARCH.md Pitfall 2, FB11984872 — `.sheet` on `MenuBarExtra` fails silently).

**Fixed width:** 280pt (matches popover — MP-01). Rendered inline via `PopoverOverlay.coursePicker`.
...
- **Header** (`.headline`, primary): "Which lecture is this?" (multi-match) or "Pick a course for this recording" (no-match).
- **Events list** (multi-match only): `List` with `.plain` style, each row shows event title (`.subheadline`, primary) + time range and location (`.caption`, secondary). Tapping a row selects that event's course and dismisses the inline overlay (`overlayState = .none`).
...
- **Picker row**: single-line `.subheadline` course title + `.caption` course code. Selected row shows `checkmark` in accent color. Tap selects + dismisses inline overlay immediately (one-tap — no separate confirm button needed; the pick IS the confirmation).
- **Create New Course button** (`.buttonStyle(.bordered)`, secondary): `plus.circle` icon + "Create New Course". Reveals the Create New Course mini-form inline (replaces the course list section within the same overlay).
...
Inline form revealed when "Create New Course" is tapped. Replaces the course list section within the same `.coursePicker` overlay (no `.sheet` transition — per RESEARCH Pitfall 2).
...
- **Create + Route button** (`.buttonStyle(.borderedProminent)`, accent): Creates `{vault}/{term}/{sanitized-code}/` folder, adds mapping to `.unibrain/courses.json`, routes recording there, dismisses inline overlay (`overlayState = .none`). Disabled when Course Code is empty.

---

### Surface 2: Manage Courses (inline overlay) (M-04)

Accessible from the idle-state popover via a "Manage Courses" button (sets `viewModel.overlayState = .manageCourses`). Editable table of the course mapping. Rendered inline within the 280pt popover — NOT a `.sheet` (per RESEARCH Pitfall 2).
...
- **Header** (`.headline`, primary): "Manage Courses".
- **Current term display** (`.subheadline` primary + `.caption` secondary): Shows `currentTerm.label` + date range. Tappable → reveals term editor inline within the same overlay (label, start date, end date fields).
...
- **Add Course Mapping button** (`.buttonStyle(.bordered)`, secondary): `plus.circle` icon. Reveals inline form: Event Title field + Course Code field + Course Name field.
- **Done button** (`.buttonStyle(.borderedProminent)`, accent): Saves changes to `.unibrain/courses.json`, dismisses inline overlay (`overlayState = .none`).
...
#### First-Time Permission Explanation (inline overlay)

Shown once (first recording where calendar Full Access is denied or only `.writeOnly` granted). Sets `viewModel.overlayState = .permissionDenied`. Stored flag: `hasShownPermissionOverlay` in `.unibrain/courses.json` or `@AppStorage`. Rendered inline within the 280pt popover — NOT a `.sheet` (per RESEARCH Pitfall 2).
...
- **Header** (`.headline`, primary): "Calendar Access Needed".
...
- **Continue with Manual Pick button** (`.buttonStyle(.bordered)`, secondary): Dismisses the inline overlay (`overlayState = .none`). The manual picker (Surface 1) appears immediately for this recording (sets `overlayState = .coursePicker(.none)`).
...
- **Tap action**: Opens macOS System Settings deep-link (same URL as first-time permission overlay). Does NOT re-show the first-time overlay — just the deep-link.
...
- **Set Term button** (`.buttonStyle(.bordered)`, accent, compact): Sets `viewModel.overlayState = .termEditor` — reveals the term editor inline (label + start date + end date fields, same mini-form pattern as Manage Courses term editing).
...
- **Progress indicator**: `ProgressView` (indeterminate) with `.caption` "Analyzing calendar…". This state is typically brief (<1s for `.single`/`.none`; the inline picker overlay appears for `.multiple`/`.none`).
...
- **Transition to picker**: When `CourseClassifier` returns `.multiple` or `.none`, the inline picker overlay (Surface 1) appears by setting `viewModel.overlayState = .coursePicker(mode)`. When `.single`, the orchestrator auto-resolves and skips to `.normalizing` → `.writing` → `.completed` (Phase 3 flow).
...
- **Term label** (`.caption`, `.secondary`): "Term: {currentTerm.label}". Tappable → sets `overlayState = .termEditor`. If term expired, shows the term-expired banner (Surface 4) instead.
- **Manage Courses button** (`.buttonStyle(.bordered)`, secondary, `.controlSize(.small)`): `folder.badge.gearshape` icon + "Manage Courses". Sets `overlayState = .manageCourses` (reveals Surface 2 inline). Small/secondary — Record is still the dominant CTA.
...
| **Permission overlay — heading** | "Calendar Access Needed" |
| **Permission overlay — body paragraph 1** | "unibrain uses your calendar to automatically route recordings to the right course folder." |
| **Permission overlay — body paragraph 2** | "Without calendar access, you'll need to pick the course manually each time you record." |
| **Permission overlay — enable button** | "Open System Settings" |
| **Permission overlay — dismiss button** | "Continue with Manual Pick" |
...
- **`.multiple` match:** orchestrator pauses at `.awaitingUserChoice`. Inline picker overlay (Surface 1, Variant A) appears via `overlayState = .coursePicker(.multiple(...))`. Angelica taps an event or course → orchestrator resumes with `.normalizing`.
- **`.none` match:** orchestrator pauses at `.awaitingUserChoice`. Inline picker overlay (Surface 1, Variant B) appears via `overlayState = .coursePicker(.none)`. Same resume flow.
...
- **Calendar denied:** First-time permission overlay (Surface 3) appears on the first recording where calendar is denied. Subsequent recordings show the ongoing banner.
- **Calendar granted later:** If Angelica grants via System Settings, the app detects it on next activation (`NSApplication.didBecomeActiveNotification` or equivalent) and the banner/overlay dismisses. No app restart needed.
...
- **From idle state:** "Manage Courses" button in the popover sets `overlayState = .manageCourses` (new in Phase 4).
...
|-----------|-------------------|--------------|--------|
| `CoursePickerView` | `UnibrainApp/Views/CoursePickerView.swift` | 120-150 | MP-01..05 |
...
| `ManageCoursesView` | `UnibrainApp/Views/ManageCoursesView.swift` | 60-80 | M-04 |
| `PermissionDeniedView` | `UnibrainApp/Views/PermissionDeniedSheet.swift` | 40-50 | P-01 |
...
**Estimated total new SwiftUI:** ~400-550 lines across ~9 new view files + ~30 lines of extensions to existing files. Within M-04's "~50-100 lines" estimate for Manage Courses alone; total Phase 4 UI is larger because it includes the picker, permission UX, and banners. All surfaces render inline via `PopoverOverlay` view-state switching — no `.sheet` usage (per RESEARCH Pitfall 2, FB11984872).
...
|----------|-------------|-------------|
| SwiftUI (system) | `PopoverOverlay` inline view-state switching, `List`, `Section`, `TextField`, `Button`, `Label`, `Alert`, `DatePicker`, `Canvas` | not required (Apple framework — no third-party audit needed) |
...
- [ ] Dimension 5 Spacing: PASS — 4pt-multiple scale inherited from Phase 3, 280pt popover/overlay width
...
*Sources: 04-CONTEXT.md (M-01..M-04, P-01..P-05, MP-01..MP-05, CT-01..CT-04) + 03-UI-SPEC.md (inherited design tokens) + Apple HIG for MenuBarExtra inline overlay/List patterns*
